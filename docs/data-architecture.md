...
- **Pre-migration snapshots:** an existing store is checkpointed and copied to `<db>.v<from>.bak` before any migration applies; a failed snapshot never blocks (fail-soft), a failed migration leaves both the store and the snapshot intact.
