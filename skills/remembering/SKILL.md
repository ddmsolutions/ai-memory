...
`lint` reports store health; `consolidate` lists raw episodes to distil (`promote <id> --type <t> --content "<standalone rewrite>"`); `decay --dry-run` previews aging; `purge --entity <x> --yes` erases a subject completely.
